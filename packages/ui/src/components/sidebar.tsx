...
>(
  (
    ...
    const [leftOpen, _setLeftOpen] = React.useState(defaultLeftOpen)
    const [rightOpen, _setRightOpen] = React.useState(defaultRightOpen)
    ...
    const value = React.useMemo(
      () => ({
        state: isMobile || (!leftOpen && !rightOpen) ? "collapsed" : "expanded",
        leftOpen,
        rightOpen,
        setLeftOpen,
        setRightOpen,
...
const sidebarVariants = cva(
  "fixed top-0 z-40 h-screen border-r bg-background transition-all duration-300",
  {
    variants: {
      side: {
        left: "left-0",
...
>(
  (
    ...
    const sheetOpen = side === "left" ? leftSheetOpen : rightSheetOpen
    const setSheetOpen = side === "left" ? setLeftSheetOpen : setRightSheetOpen
