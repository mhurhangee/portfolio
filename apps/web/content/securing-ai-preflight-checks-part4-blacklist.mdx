...
tags: ['jailbreak-prevention', 'ai-security', 'blacklist-detection', 'content-moderation', 'llm-security', 'typescript']
...
function tokenBasedAnalysis(input) {
  ...
  const targetWords = new Set(['instructions', 'limitations', 'rules', 'guidelines', 'restrictions']);
