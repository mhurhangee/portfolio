...
tags: ['preflight-checks', 'system-architecture', 'implementation', 'best-practices', 'performance-optimization']
...
import { enhancedRateLimitCheck, rateLimitUserCheck, rateLimitGlobalCheck } from './checks/rate-limiting';
...
export async function processPreflightResults(
  ...
  const warningResults = results.filter(result => 
    ['blacklisted_keywords', 'moderation_flagged', 'ethical_concerns']
...
import { runPreflightChecks, processPreflightResults } from '../lib/preflight-checks/preflight-checks';
...
function getUserFriendlyMessage(code: string): string {
  const messages = {
    ...
    'unsupported_language': 'This service currently only supports English. Please submit your request in English.',
    ...
    'moderation_flagged': 'Your message was flagged by our content filter. Please revise and try again.',
