...
export function TableOfContents() {
  ...
  useEffect(() => {
    ...
    const articleHeadings = Array.from(document.querySelectorAll('article h1, article h2, article h3'))
      .map((element) => {
        const id = element.id || element.textContent?.toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, '-') || '';
