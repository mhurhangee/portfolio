  ...
  run: async ({ lastMessage, checkConfig, logger }: PreflightParams) => {
    try {
      ...
      if (!analysis.overall.safeToProcess) {
        ...
        if (analysis.jailbreak.isAttempt && analysis.jailbreak.confidence > config.jailbreakThreshold) {
          code = 'jailbreak_attempt';
          shouldBlock = config.blockJailbreakAttempts;
        } else if (analysis.ethical.hasConcerns && 
        ...
        } else if (analysis.copyright.potentialIssue && 
