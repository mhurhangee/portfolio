...
export const contentModerationCheck: PreflightCheck = {
  ...
  defaultConfig: {
    ...
    configurableCategories: ['sexual/minors', 'hate/threatening', 'violence/graphic', 'self-harm/intent', 'harassment/threatening'],
    // Default threshold for flagging content (0-1)
    threshold: 0.8
  },
  run: async ({ lastMessage, checkConfig, logger, conversationContext }: PreflightParams): Promise<CheckResult> => {
    try {
      // Get configuration, using defaults if not provided
      const config = {
        model: checkConfig?.model ?? "text-moderation-latest",
        strictCategories: checkConfig?.strictCategories ?? ['sexual', 'hate', 'violence', 'self-harm', 'harassment'],
        configurableCategories: checkConfig?.configurableCategories ?? ['sexual/minors', 'hate/threatening', 'violence/graphic', 'self-harm/intent', 'harassment/threatening'],
      ...
      if (result.flagged) {
        ...
        Object.entries(result.categories).forEach(([category, flagged]) => {
          ...
          if (result.category_scores && category in result.category_scores) {
            categoryScores[category] = result.category_scores[category as keyof typeof result.category_scores];
          }
        });
