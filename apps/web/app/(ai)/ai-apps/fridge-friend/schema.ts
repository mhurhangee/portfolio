...
export const recipeSuggestionSchema = z.object({
  ...
  ingredientsUsed: z.array(z.string()).describe("List of ingredients from user's input that are used in this recipe"),
  ...
  matchPercentage: z.number().min(0).max(100).describe("Percentage of user's ingredients used in this recipe")
});

// Define the schema for recipe suggestions response
export const recipeSuggestionsSchema = z.object({
  suggestions: z.array(recipeSuggestionSchema).describe("List of recipe suggestions based on ingredients")
});
...
export const recipeSchema = z.object({
  ...
  ingredients: z.array(
    z.object({
      ...
      fromUserInput: z.boolean().describe("Whether this ingredient was in the user's original input")
    })
