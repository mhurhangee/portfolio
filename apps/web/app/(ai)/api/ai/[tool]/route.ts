  ...
  try {
    ...
    try {
      ...
      if (typeof toolModule.POST === 'function') {
        ...
        return NextResponse.json(
          { error: `API endpoint for '${toolSlug}' does not support POST requests`, code: 'method_not_supported' },
          { status: 405 }
        );
      }
    } catch (importError) {
      ...
      return NextResponse.json(
        { error: `AI tool '${toolSlug}' implementation not found`, code: 'implementation_not_found' },
        { status: 404 }
      );
    }
  } catch (error) {
